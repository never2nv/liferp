...
function MoneyTab()
	...
	FirstTabPanel:EnableVerticalScrollbar( true )
		function FirstTabPanel:Update()
			...
			MoneyCat:SetLabel("Money/Trading")
			...
			Commands:SetLabel("Actions")
					...
					ActionsPanel:SetSize(740,210)
                       ...
                       -- Change rpname
						...
						-- Advert
					    local AdvertButton = ActionsPanel:Add("DButton")
				        AdvertButton:SetText(LANGUAGE.advertbut)
				        AdvertButton.DoClick = function()
					    Derma_StringRequest("Broadcast Advert", "Input advert text", "", function(a) LocalPlayer():ConCommand("say /advert " .. tostring(a)) end)
				        end
						
						-- Place a hit
						local hitmenu = ActionsPanel:Add("DButton")
						hitmenu:SetText(LANGUAGE.hitbutton)
						hitmenu.DoClick = function()
							LocalPlayer():ConCommand("hitmenu")
						end	
						...
						local report = ActionsPanel:Add("DButton")
						report:SetText(LANGUAGE.report)
...
--?? This shouldn't be called immediately. InitClient needs to run first.
--RunConsoleCommand("_initme")
	...
	for k,v in SortedPairs( LocalPlayer().Skills ) do
		...
		SkillBuy.OnCursorEntered = function(s)
			...
			if sk.pricetbl[mylvl+1] then
				...
				SkillBuy:SetToolTip("Your level is maxxed out broski, whoa!")
			end
			return OldEnter(s)
		end
		SkillBuy.DoClick = function()
			...
			if currentExp < sk.exptbl[v.lvl] then --v.exp contains old information, have to get it fresh from the skills table.
				LocalPlayer():ChatPrint("You need more experience, lol you noob!")
				chat.PlaySound()
			else
