...
hook.Add("DatabaseInitialized", "Load NPC poses from DB", LoadNPCs)

--Old method of loading NPCs.
--[[if file.Exists("LDRP_NPCPositions.txt", "DATA") then
	LDRP.SavedPoses = von.deserialize(file.Read("LDRP_NPCPositions.txt"))
else
	GM:WriteOut("NPC positions file was not found! No NPCs will be spawned.", Severity.Info)
end]]
...
function LDRP.SetNPCPos(ply,cmd,args)
	...
	--file.Write("LDRP_NPCPositions.txt", von.serialize(LDRP.SavedPoses))
...
function LDRP.RepeatTut(ply,cmd,args)
	PrintTable(LDRP_SH.ShopPoses)
	if !LDRP_SH.ShopPoses["Tutorial Lady"] or ply:GetPos():Distance(LDRP_SH.ShopPoses["Tutorial Lady"]) > 300 then print("Not running") return end
	print("Running tut")
...
function LDRP.SaveProps()
	file.Write("liquiddrp/savedprops.txt",glon.encode(LDRP.SavedProps))
	file.Write("liquiddrp/savedremove.txt",glon.encode(LDRP.SavedRemove))
end
...
function LDRP.MapSetupCMD(ply,cmd,args)
	...
	if args[1] == "addprop" then
		if LDRP.SavedInMap[ent] then
			...
			ent:Remove()
			ply:ChatPrint("Removed saved prop.")
		else
